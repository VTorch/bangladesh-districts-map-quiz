...
let names = [], remaining = [];
let correct = 0, attempts = 0, tries = 0;
let currentTarget = null, startTime = Date.now(), timerInterval;
...
function blinkRed(layer) {
  ...
  const iv = setInterval(() => {
    ...
    on = !on;
    if (++count>=6) { clearInterval(iv); layer.setStyle({ fillColor:REVEALED_COLOR }); }
  },200);
}
...
function handleClick(feature, layer) {
  if (layer.answered) {
    ...
    setTimeout(()=>{
      t.style.opacity=0;
      setTimeout(()=>{ layer.closeTooltip(); layer.unbindTooltip(); },200);
    },500);
    return;
  }
  ...
  if (feature.properties.ADM2_EN===currentTarget) {
    ...
    tries++;
    if (tries>=3) {
      const targ=featuresByName[currentTarget];
      targ.answered=true;
      blinkRed(targ);
...
function nextQuestion(delay=0) {
  setTimeout(()=>{
    districtLayer.eachLayer(l=>{ if(!l.answered) l.setStyle(styleDefault()); });
    if (!remaining.length) {
      currentTarget=null;
...
map.getContainer().addEventListener("mousedown",e=>{ if(e.button===1) map.getContainer().style.cursor="grabbing"; });
map.getContainer().addEventListener("mouseup",  e=>{ if(e.button===1) map.getContainer().style.cursor=""; });
map.getContainer().addEventListener("mousemove",e=>{
  const tt=document.getElementById("cursor-tooltip");
  tt.style.left=(e.clientX+12)+"px"; tt.style.top=(e.clientY+12)+"px";
});

// load data & build quiz
  ...
  .then(geojson=>{
    ...
    districtLayer = L.geoJSON(geojson, {
      ...
      onEachFeature:(f,l)=>{
        ...
        featuresByName[name]=l; l.answered=false;
    ...
    const b=districtLayer.getBounds();
    map.fitBounds(b);
    const iz=map.getZoom();
    map.setMinZoom(iz);
